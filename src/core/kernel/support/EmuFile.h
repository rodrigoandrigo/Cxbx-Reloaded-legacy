...
inline ::HANDLE CxbxCreateHostFile(
    ...
    DWORD flagsAndAttributes)
{
    ...
    parameters.dwFileAttributes = flagsAndAttributes & 0x0000FFFFu;
    parameters.dwFileFlags = flagsAndAttributes & 0xFFF00000u;
    parameters.dwSecurityQosFlags = flagsAndAttributes & 0x000F0000u;
    return CreateFile2FromAppW(path.c_str(), desiredAccess, shareMode,
        creationDisposition, &parameters);
#else
    return CreateFileW(path.c_str(), desiredAccess, shareMode, nullptr,
...
{
#if defined(CXBXR_UWP)
    return DeleteFileFromAppW(path.c_str());
#else
