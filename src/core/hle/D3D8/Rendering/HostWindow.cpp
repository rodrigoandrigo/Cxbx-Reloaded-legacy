...
{
#if defined(CXBXR_UWP)
	// XAML owns the window and message pump. RenderBase remains an in-process
	// backend coordinator, but no HWND or Win32 ImGui platform backend exists.
...
{
#if defined(CXBXR_UWP)
	EmuLog(LOG_LEVEL::ERROR, "Xbox fatal error %d: %s", g_CxbxFatalErrorCode, uem_str.c_str());
	return;
#else
