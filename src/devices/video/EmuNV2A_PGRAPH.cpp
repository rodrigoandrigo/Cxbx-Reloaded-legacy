...
static bool pgraph_trace_can_read(const void* address, size_t size)
{
...
static void pgraph_trace_ensure_device()
{
	...
	if (pgraph_trace_can_read(pDeviceGlobal, 4)) {
		uint32_t devAddr = *(uint32_t*)pDeviceGlobal;
		auto* deviceObject = reinterpret_cast<void*>(static_cast<uintptr_t>(devAddr));
		if (pgraph_trace_can_read(deviceObject, 4)) {
			pgraph_trace_pDeviceObj = reinterpret_cast<uint32_t**>(deviceObject);
		}
	}
}
