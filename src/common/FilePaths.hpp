...
// UWP has no unrestricted current directory. The UWP host supplies the path
// of ApplicationData::Current->LocalFolder before the emulation core starts.
// Keeping this as UTF-8 avoids coupling the core to C++/CX.
void CxbxrSetBrokeredDataPath(const std::string& local_folder_path);

// Content selected through a UWP FolderPicker. The host must retain the
// matching brokered StorageFolder for the lifetime of emulation.
